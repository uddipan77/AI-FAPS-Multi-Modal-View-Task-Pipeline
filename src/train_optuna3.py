...
import lightning.pytorch.loggers as pl_loggers  # for logger; we'll override this to return None
...
def create_callbacks(cfg):
    ...
    # Remove ModelCheckpoint callback instantiation (checkpointing)
    # EarlyStopping
...
def instantiate_logger(cfg):
    """
    We now simply return None to avoid using any external logger (e.g., Wandb).
...
def train_once(config, trial=None):
    ...
    Args:
        ...
        trial (optuna.trial.Trial, optional): The Optuna trial object.
...
def objective(trial, base_config):
    """
    Modify base_config in-place with trial.suggest_*, then call train_once(base_config).
...
def main():
    ...
    best_params_file = "optuna_best_params_dense121_IT.yaml"
