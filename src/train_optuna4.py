...
It handles configuration instantiation, callback and logger creation, training, and evaluation of
a multi-task model. Hyperparameters are tuned via Optuna and the best parameters/metric are saved
to a YAML file.
"""
...
# We no longer import ModelCheckpoint since checkpointing is removed.
# We also override the logger to return None.
# Note: We still import pl_loggers if needed, but instantiate_logger returns None.
...
def instantiate_recursive(cfg):
    """
    Recursively walk through 'cfg'.
    If you see a dict with '_target_', import and instantiate that object.
    If '_partial_' == true, we return a functools.partial instead of a full instance.
    This allows the model to supply 'params' later (e.g. for torch.optim.*).

    Args:
        cfg (dict, list, or any): The configuration structure.
...
def parse_args():
    ...
    parser.add_argument("--study_name", type=str, default="optuna_study", help="Optuna study name")
...
def load_config(cfg_path: str):
    """
    Loads a YAML config file and returns a raw dictionary.
...
def create_callbacks(cfg):
    """
    Create a list of Lightning callbacks based on the provided configuration.
    ...
    # We no longer include ModelCheckpoint since checkpointing is removed.
    # EarlyStopping
...
def instantiate_logger(cfg):
    """
    We return None to disable any external logging (e.g., Wandb).
...
def train_once(config, trial=None):
    """
    1) Recursively instantiate config => _target_ => actual objects or partials.
    2) Build DataModule, Model, Trainer.
    3) Run fit/test.
    4) Return metric.

    Optionally attach a pruning callback if trial is provided.
...
def objective(trial, base_config):
    """
    Modify base_config in-place with trial.suggest_*,
    then call train_once(base_config).
    ...
    Returns:
        float: The metric value resulting from the training trial.
    """
    ...
    # 5) Batch size (User wants only 8 or 16)
    ...
    gaussian_blur_kernel = trial.suggest_categorical("gaussian_blur_kernel", [13, 15, 17, 19, 21])
...
def main():
    ...
    Loads the configuration from a YAML file, creates an Optuna study,
    runs the optimization over a number of trials, prints the best parameters
    and metric, and saves them to a YAML file.
    """
    ...
    best_params_file = "optuna_best_params_Dino_ITplus.yaml"
